...
class DQN_agent:
	def __init__(self, 
		...
		if os.path.exists('models/target_model'):
			...
			self.policy_net = DQN(self.state_size, number_of_layers).to(device)
			self.target_net = DQN(self.state_size, number_of_layers).to(device)
	...
	def optimize(self, step):
		...
		self.last_train_loss = loss.cpu().detach().numpy()
		self.training_losses.append(self.last_train_loss.item())
		self.optimizer.step()
		...
		self.last_validation_loss = loss_val.cpu().detach().numpy()
		self.validation_losses.append(self.last_validation_loss.item())
